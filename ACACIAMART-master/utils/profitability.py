...
def product_revenue(order):
    """Calculate product sales only; exclude delivery charges."""
...
def get_paid_sales():
    ...
    try:
        ...
        orders_response = requests.get(
            f"{SUPABASE_URL}/rest/v1/orders?select=total,total_charged,subtotal,discount,shipping,items,status",
        ...
        if orders_response.status_code == 200:
            ...
            for order in orders:
                if order.get('status') != 'cancelled':
                    total_orders += product_revenue(order)
        else:
...
def get_monthly_profitability(year=None):
    ...
    try:
        ...
        # Get orders (Cash + POS)
        orders_response = requests.get(
            f"{SUPABASE_URL}/rest/v1/orders?select=total,total_charged,subtotal,discount,shipping,created_at,status,items",
            headers=SUPABASE_HEADERS,
            timeout=30
        )
        ...
        if orders_response.status_code == 200:
            ...
            for order in orders:
                ...
                months[month_key]['cash_pos_sales'] += product_revenue(order)
