            ...
            if isinstance(data, list):
                ...
                for order in data:
                    ...
                    if 'order_id' in order:
                        ...
                        # Keep one copy when a retried payment request created
                        # duplicate rows with the same client order ID.
...
def get_sales_analytics():
    ...
    try:
        ...
        for order in orders:
            ...
            if customer_name in customer_data:
                customer_data[customer_name]['total_spent'] += order_subtotal - float(order.get('discount', 0) or 0)
